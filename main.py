...
from model import ConvE, DistMult, Complex, DistMultDecompress, ComplexDecompress, EnDistMultDecompress, RelDistMultDecompress, LDecomDistMult, LEnDecomDistMult, LRelDecomDistMult, LComplexDeComplex
from model import EnComplexDecompress
...
model_name = '{2}_{0}_{1}_{3}'.format(Config.input_dropout, Config.dropout, Config.model_name, Config.learning_rate)
log_name = '{2}_{0}_{1}_{3}.txt'.format(Config.input_dropout, Config.dropout, Config.model_name, Config.learning_rate)
print(log_name)
...
def main():
    ...
    elif Config.model_name == 'DistMultDecompress':
        model = DistMultDecompress(vocab['e1'].num_token, vocab['rel'].num_token)
    elif Config.model_name == 'EnDistMultDecompress':
        model = EnDistMultDecompress(vocab['e1'].num_token, vocab['rel'].num_token)
    elif Config.model_name == 'RelDistMultDecompress':
        model = RelDistMultDecompress(vocab['e1'].num_token, vocab['rel'].num_token)
    elif Config.model_name == 'ComplexDecompress':
        model = ComplexDecompress(vocab['e1'].num_token, vocab['rel'].num_token)
    elif Config.model_name == 'LDecomDistMult':
        model = LDecomDistMult(vocab['e1'].num_token, vocab['rel'].num_token)
    elif Config.model_name == 'LEnDecomDistMult':
        model = LEnDecomDistMult(vocab['e1'].num_token, vocab['rel'].num_token)
    elif Config.model_name == 'LRelDecomDistMult':
        model = LRelDecomDistMult(vocab['e1'].num_token, vocab['rel'].num_token)
    elif Config.model_name == 'LDeComplex':
        model = LDeComplex(vocab['e1'].num_token, vocab['rel'].num_token)
    elif Config.model_name == 'EnComplexDecompress':
        model = EnComplexDecompress(vocab['e1'].num_token, vocab['rel'].num_token)
    else:
        print('Unknown model: {0}', Config.model_name)
        raise Exception("Unknown model!")
    print('num of paras is %d' %sum(p.numel() for p in model.parameters()))
    ...
    for epoch in range(epochs):
        ...
        #     dev_mrr = ranking_and_hits(model, dev_rank_batcher, vocab, 'dev_evaluation', log_name, epoch)
        ...
        with torch.no_grad():
            ...
            if dev_mrr > best_mrr:
                start_time = time.time()
                best_mrr = dev_mrr
                ranking_and_hits(model, test_rank_batcher, vocab, 'test_evaluation', log_name, epoch)
