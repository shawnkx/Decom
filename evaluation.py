...
def ranking_and_hits(model, dev_rank_batcher, vocab, name, log_file=None, epoch=0):
    ...
    for i, str2var in enumerate(dev_rank_batcher):
        #print(str2var['e1'].shape, str2var['e2'].shape)
        total_triples += str2var['e1'].shape[0]
        e1 = str2var['e1']
        e2 = str2var['e2']
        rel = str2var['rel']
    ...
    log.info('decoding speed: {0}'.format(total_triples / (time.time() -start_time)))
    #print(total_triples)
    if log_file:
        with open(log_file, 'a+') as f:
            f.write('Mean reciprocal rank: {0}'.format(np.mean(1./np.array(ranks))) + '\n')
            f.write('decoding speed: {0}'.format(total_triples / (time.time() -start_time)))
    #exit()
